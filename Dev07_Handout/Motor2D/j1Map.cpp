...
void j1Map::Draw()
{
	...
	//MapLayer* layers = this->data.layers.start->data;
	p2List_item<MapLayer*>* layers = this->data.layers.start;
	
	//MapLayer* layers = layers
	for (layers; layers == NULL; layers.next)
	{
...
TileSet* j1Map::GetTilesetFromTileId(int id) const
{
	...
	{
		TileSet* s = item->data;
		if (s->firstgid <= id && id < item->next->data->firstgid)
			return s;
		else
		item = item->next;
	}
